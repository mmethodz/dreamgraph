...
exports.GraphSignalProvider = void 0;
...
class GraphSignalProvider {
    ...
    _onSignal = new vscode.EventEmitter();
    onSignal = this._onSignal.event;
    /** Status bar item showing graph awareness */
    _statusItem;
    constructor(_mcpClient, _daemonClient) {
        this._mcpClient = _mcpClient;
        this._daemonClient = _daemonClient;
    ...
    async _fetchSignal(uri) {
        ...
        try {
            ...
            if (this._cache.size > 50) {
                const oldest = [...this._cache.entries()].sort((a, b) => a[1].fetchedAt.getTime() - b[1].fetchedAt.getTime());
    ...
    /* ---- Status Bar ---- */
    _updateStatusBar(signal) {
        const parts = [];
        if (signal.tensionCount > 0)
            parts.push(`⚡${signal.tensionCount}`);
        if (signal.insightCount > 0)
            parts.push(`💡${signal.insightCount}`);
        if (signal.adrCount > 0)
            parts.push(`📋${signal.adrCount}`);
        if (signal.featureCount > 0)
            parts.push(`🔷${signal.featureCount}`);
        if (parts.length > 0) {
...
exports.GraphSignalProvider = GraphSignalProvider;
