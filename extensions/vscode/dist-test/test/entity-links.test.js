...
function formatEntityLabel(scheme, name) {
    const icons = {
...
function replaceEntityUris(text) {
...
function linkifyEntities(html) {
    ...
    for (let i = 0; i < preParts.length; i += 2) {
        if (i % 2 === 0)
            preParts[i] = linkifyOutsidePre(preParts[i]);
    }
    return preParts.join('');
}
// ── Tests ─────────────────────────────────────────────────────────────────────
(0, node_test_1.default)('T-2.1: feature:// URIs are linkified', () => {
    const html = '<p>See feature://my_feature for details.</p>';
    const result = linkifyEntities(html);
    strict_1.default.match(result, /class="entity-link"/);
    strict_1.default.match(result, /data-type="feature"/);
    strict_1.default.match(result, /data-uri="feature:\/\/my_feature"/);
});
(0, node_test_1.default)('T-2.2: all recognised schemes are linkified', () => {
    const schemes = ['feature', 'workflow', 'data-model', 'entity', 'adr', 'tension', 'edge', 'file'];
    for (const scheme of schemes) {
        const html = `<p>Link: ${scheme}://some-name here.</p>`;
        const result = linkifyEntities(html);
        strict_1.default.match(result, new RegExp(`data-type="${scheme}"`), `scheme ${scheme} should be linkified`);
    }
});
(0, node_test_1.default)('T-2.3: generated anchor has correct data-type and data-uri', () => {
    const html = '<p>workflow://dream_cycle</p>';
    const result = linkifyEntities(html);
    strict_1.default.match(result, /data-type="workflow"/);
    strict_1.default.match(result, /data-uri="workflow:\/\/dream_cycle"/);
    strict_1.default.match(result, /href="#"/);
    strict_1.default.match(result, /class="entity-link"/);
});
(0, node_test_1.default)('T-2.4: URIs inside <pre>…</pre> are NOT linkified', () => {
    const html = '<pre><code>feature://do_not_link</code></pre>';
    const result = linkifyEntities(html);
    strict_1.default.doesNotMatch(result, /class="entity-link"/);
    strict_1.default.match(result, /feature:\/\/do_not_link/); // original text preserved
});
(0, node_test_1.default)('T-2.4b: URIs inside inline <code>…</code> are NOT linkified', () => {
    const html = '<p>Use <code>feature://my_feature</code> in prose.</p>';
    const result = linkifyEntities(html);
    strict_1.default.doesNotMatch(result, /class="entity-link"/);
    strict_1.default.match(result, /feature:\/\/my_feature/);
});
(0, node_test_1.default)('T-2.5: URIs inside existing href="" are NOT double-linkified', () => {
    ...
    strict_1.default.equal(anchorCount, 0, 'href content should not be linkified');
});
(0, node_test_1.default)('T-S1.4: javascript: scheme is NOT linkified', () => {
    const html = '<p>javascript://alert(1)</p>';
    const result = linkifyEntities(html);
    strict_1.default.doesNotMatch(result, /class="entity-link"/);
});
(0, node_test_1.default)('T-S1.4b: data: scheme is NOT linkified', () => {
    const html = '<p>data://something</p>';
    const result = linkifyEntities(html);
    strict_1.default.doesNotMatch(result, /class="entity-link"/);
});
(0, node_test_1.default)('T-S1.4c: vscode: scheme is NOT linkified', () => {
    const html = '<p>vscode://extension/something</p>';
    const result = linkifyEntities(html);
    strict_1.default.doesNotMatch(result, /class="entity-link"/);
});
(0, node_test_1.default)('T-S3.1: data-uri only contains the allowlisted URI, not injected content', () => {
    ...
    strict_1.default.match(result, /data-uri="feature:\/\/name"/);
    ...
    for (const tag of tagMatches) {
        strict_1.default.doesNotMatch(tag, /onmouseover/, `onmouseover must not appear in tag: ${tag}`);
    }
});
(0, node_test_1.default)('T-2: multiple entity links in one paragraph', () => {
    ...
    strict_1.default.match(result, /data-type="feature"/);
    strict_1.default.match(result, /data-type="adr"/);
});
(0, node_test_1.default)('T-2: plain text outside tags is linkified', () => {
    // Even bare text not wrapped in HTML tags should be linkified
    const html = 'tension://some-tension is unresolved.';
    const result = linkifyEntities(html);
    strict_1.default.match(result, /class="entity-link"/);
    strict_1.default.match(result, /data-type="tension"/);
});
//# sourceMappingURL=entity-links.test.js.map