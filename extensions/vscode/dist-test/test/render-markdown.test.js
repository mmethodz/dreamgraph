...
// ── T-1.x: Basic markdown elements ────────────────────────────────────────
(0, node_test_1.suite)('Markdown Rendering — T-1.x', () => {
    (0, node_test_1.test)('T-1.1: bold, italic, inline code', () => {
    ...
    (0, node_test_1.test)('T-1.2: fenced code block with language class', () => {
        const html = md.render('```ts\nconst x = 1;\n```');
        strict_1.default.match(html, /<pre><code class="language-ts">/);
    });
    (0, node_test_1.test)('T-1.3: fenced code block without language still produces pre>code', () => {
        const html = md.render('```\nplain block\n```');
        strict_1.default.match(html, /<pre><code>/);
    });
    (0, node_test_1.test)('T-1.4: headings h2 and h3', () => {
        const html = md.render('## Heading 2\n### Heading 3');
        strict_1.default.match(html, /<h2>Heading 2<\/h2>/);
        strict_1.default.match(html, /<h3>Heading 3<\/h3>/);
    });
    (0, node_test_1.test)('T-1.5: tables produce table/th/td', () => {
        const html = md.render('| A | B |\n|---|---|\n| 1 | 2 |');
        strict_1.default.match(html, /<table>/);
        strict_1.default.match(html, /<th>A<\/th>/);
        strict_1.default.match(html, /<td>1<\/td>/);
    });
    (0, node_test_1.test)('T-1.6: unordered and ordered lists', () => {
        const html = md.render('- a\n- b\n\n1. c\n2. d');
        strict_1.default.match(html, /<ul>/);
        strict_1.default.match(html, /<ol>/);
        strict_1.default.match(html, /<li>/);
    });
    (0, node_test_1.test)('T-1.7: partial markdown (incomplete bold) does not produce unclosed tag', () => {
        const html = md.render('**bol');
        // markdown-it should close the tag or escape it — no raw unclosed <strong at EOL
        strict_1.default.doesNotMatch(html, /<strong>[^<]*$/);
    });
    (0, node_test_1.test)('T-1.9: external links get rendered as <a href>', () => {
        const html = md.render('[click](https://example.com)');
        strict_1.default.match(html, /<a href="https:\/\/example\.com"/);
    });
});
// ── T-1.8.x: XSS prevention via html:false ────────────────────────────────
(0, node_test_1.suite)('XSS Prevention — T-1.8.x (markdown-it html:false)', () => {
    (0, node_test_1.test)('T-1.8: raw script tag is escaped, not passed through', () => {
        const html = md.render('<script>alert("xss")</script>');
        strict_1.default.doesNotMatch(html, /<script>/);
    });
    (0, node_test_1.test)('T-1.8b: javascript: URI in link is not rendered as active href', () => {
        const html = md.render('[click](javascript:alert(1))');
        // markdown-it with linkify disabled for javascript: — should not emit href=javascript:
        strict_1.default.doesNotMatch(html, /href="javascript:/);
    });
    (0, node_test_1.test)('T-1.8c: onerror attribute in raw img tag is escaped', () => {
        const html = md.render('<img src=x onerror="alert(1)">');
        // With html:false, the tag itself is escaped as text, onerror not present
        strict_1.default.doesNotMatch(html, /onerror/);
    });
    (0, node_test_1.test)('T-S3.4: rendered output contains no active <script> tags', () => {
        const inputs = [
            '# heading\n**bold** `code`',
            '```js\nconsole.log("hi")\n```',
            '| col |\n|---|\n| val |',
